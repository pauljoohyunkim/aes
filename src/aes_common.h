/*
    This header files contains some of the functions and variables
    that all of the AES encryption methods need.
*/


uint8_t buffer[16] = {0};       //For data
uint8_t key[32] = {0};          //For key
uint8_t expanded_key[480];      //Size will be (keylen) * (number of round keys). https://en.wikipedia.org/wiki/AES_key_schedule
int nRoundKeys;                 //For key scheduling
int nWords;                     //For key scheduling
int nRounds;                    //Number of rounds: 10 for AES128, 12 for AES192, 14 for AES256
int keylen = 0;                 //Length of key (16 for AES128, 24 for AES192, 32 for AES256) 

//  DATA

// Rijndael S-box
// https://en.wikipedia.org/wiki/Rijndael_S-box
uint8_t sbox[256] = {};

// rc value for round constant
uint8_t rc[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
// Round constants generation
uint8_t rcon[10][4] = {0};


//Galois Multiplication Table
/*
    Galois multiplication is bitwise polynomial multiplication mod x^8 + x^4 + x^3 + x + 1.
    You take two numbers, express them into bits, and write them as polynomials with
    the bits as coefficients, multiply the polynomials, and take the remainder as you divide
    by the 8th degree polynomial given.
    Since this operation is quite tedious to program, the following loop-up tables are used to
    find the result when multiplied by 2 and 3.
*/
uint8_t table_2[256] = {};
uint8_t table_3[256] = {};


//  FUNCTIONS

//Declaration of functions
void aes();
void key_schedule();
void addWord(uint8_t* destWord, uint8_t* wordA, uint8_t* wordB);
void subWord(uint8_t* destWord, uint8_t* inWord);
void subRotWord(uint8_t* destWord, uint8_t* inWord);
void addRoundKey(uint8_t* key);
void subBytes();
void permutation();
void mult();



void aes()
{
    key_schedule();
    //Add round key
    addRoundKey(expanded_key);
    //SubBytes step
    for(int i = 1; i < nRounds; i++)
    {
        subBytes();
        //Permutation step
        permutation();
        //Multiplication step
        mult();
        //Adding round key
        addRoundKey(expanded_key + 16 * i);
    }

    //Last round does not require mult step.
    subBytes();
    permutation();
    addRoundKey(expanded_key + 16 * nRounds);
}

//Generates expanded key
void key_schedule()
{
    // The first "keylen" bytes are the same
    for(int i = 0; i < keylen; i++)
    {
        expanded_key[i] = key[i];
    }

    
    if(nWords <= 6)
    {
        for(int i = nWords; i < 4 * nRoundKeys; i++)
        {
            if(i % nWords == 0)
            {
                subRotWord(expanded_key + 4 * i, expanded_key + 4 * (i - 1));
                addWord(expanded_key + 4 * i, expanded_key + 4 * i, expanded_key + 4 * (i - nWords));
                addWord(expanded_key + 4 * i, expanded_key + 4 * i, rcon[i / nWords - 1]);
            }
            else
            {
                addWord(expanded_key + 4 * i, expanded_key + 4 * (i - nWords), expanded_key + 4 * (i - 1));
            }
        }
    }
    // AES256 needs special treatment since 8 words
    else
    {
        for(int i = nWords; i < 4 * nRoundKeys; i++)
        {
            if(i % nWords == 0)
            {
                subRotWord(expanded_key + 4 * i, expanded_key + 4 * (i - 1));
                addWord(expanded_key + 4 * i, expanded_key + 4 * i, expanded_key + 4 * (i - nWords));
                addWord(expanded_key + 4 * i, expanded_key + 4 * i, rcon[i / nWords - 1]);
            }
            else if (i % nWords == 4)
            {
                subWord(expanded_key + 4 * i, expanded_key + 4 * (i - 1));
                addWord(expanded_key + 4 * i, expanded_key + 4 * (i - nWords), expanded_key + 4 * i);
            }
            else
            {
                addWord(expanded_key + 4 * i, expanded_key + 4 * (i - nWords), expanded_key + 4 * (i - 1));
            }
        }
    }
    
}

// Key-schedule: xor-ing two words
void addWord(uint8_t* destWord, uint8_t* wordA, uint8_t* wordB)
{
    destWord[0] = wordA[0] ^ wordB[0];
    destWord[1] = wordA[1] ^ wordB[1];
    destWord[2] = wordA[2] ^ wordB[2];
    destWord[3] = wordA[3] ^ wordB[3];
}

// Key-schedule: substitution of the word
void subWord(uint8_t* destWord, uint8_t* inWord)
{
    destWord[0] = sbox[inWord[0]];
    destWord[1] = sbox[inWord[1]];
    destWord[2] = sbox[inWord[2]];
    destWord[3] = sbox[inWord[3]];
}

// Key-schedule: one-byte left circular shift, then substitution
void subRotWord(uint8_t* destWord, uint8_t* inWord)
{
    destWord[0] = sbox[inWord[1]];
    destWord[1] = sbox[inWord[2]];
    destWord[2] = sbox[inWord[3]];
    destWord[3] = sbox[inWord[0]];
}

// Adding 16 bit of the key
void addRoundKey(uint8_t* key)
{
    for(int i = 0; i < 16; i++)
    {
        buffer[i] = buffer[i] ^ key[i];
    }
}

//https://en.wikipedia.org/wiki/Advanced_Encryption_Standard#The_SubBytes_step
void subBytes()
{
    for(int i = 0; i < 16; i++)
    {
        buffer[i] = sbox[buffer[i]];
    }
}

// This is known as ShiftRows step on https://en.wikipedia.org/wiki/Advanced_Encryption_Standard
void permutation()
{
    //Second row
    uint8_t temp[4];
    temp[0] = buffer[1];
    temp[1] = buffer[5];
    temp[2] = buffer[9];
    temp[3] = buffer[13];
    buffer[1] = temp[1];
    buffer[5] = temp[2];
    buffer[9] = temp[3];
    buffer[13] = temp[0];

    //Third row
    temp[0] = buffer[2];
    temp[1] = buffer[6];
    temp[2] = buffer[10];
    temp[3] = buffer[14];
    buffer[2] = temp[2];
    buffer[6] = temp[3];
    buffer[10] = temp[0];
    buffer[14] = temp[1];

    //Fourth row
    temp[0] = buffer[3];
    temp[1] = buffer[7];
    temp[2] = buffer[11];
    temp[3] = buffer[15];
    buffer[3] = temp[3];
    buffer[7] = temp[0];
    buffer[11] = temp[1];
    buffer[15] = temp[2];
}

void mult()
{
    //Copy buffer to temp buffer
    uint8_t temp[16];
    for(int i = 0; i < 16; i++)
    {
        temp[i] = buffer[i];
    }

    buffer[0] = table_2[temp[0]] ^ table_3[temp[1]] ^ temp[2] ^ temp[3];
    buffer[1] = temp[0] ^ table_2[temp[1]] ^ table_3[temp[2]] ^ temp[3];
    buffer[2] = temp[0] ^ temp[1] ^ table_2[temp[2]] ^ table_3[temp[3]];
    buffer[3] = table_3[temp[0]] ^ temp[1] ^ temp[2] ^ table_2[temp[3]];
    buffer[4] = table_2[temp[4]] ^ table_3[temp[5]] ^ temp[6] ^ temp[7];
    buffer[5] = temp[4] ^ table_2[temp[5]] ^ table_3[temp[6]] ^ temp[7];
    buffer[6] = temp[4] ^ temp[5] ^ table_2[temp[6]] ^ table_3[temp[7]];
    buffer[7] = table_3[temp[4]] ^ temp[5] ^ temp[6] ^ table_2[temp[7]];
    buffer[8] = table_2[temp[8]] ^ table_3[temp[9]] ^ temp[10] ^ temp[11];
    buffer[9] = temp[8] ^ table_2[temp[9]] ^ table_3[temp[10]] ^ temp[11];
    buffer[10] = temp[8] ^ temp[9] ^ table_2[temp[10]] ^ table_3[temp[11]];
    buffer[11] = table_3[temp[8]] ^ temp[9] ^ temp[10] ^ table_2[temp[11]];
    buffer[12] = table_2[temp[12]] ^ table_3[temp[13]] ^ temp[14] ^ temp[15];
    buffer[13] = temp[12] ^ table_2[temp[13]] ^ table_3[temp[14]] ^ temp[15];
    buffer[14] = temp[12] ^ temp[13] ^ table_2[temp[14]] ^ table_3[temp[15]];
    buffer[15] = table_3[temp[12]] ^ temp[13] ^ temp[14] ^ table_2[temp[15]];

}